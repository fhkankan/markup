...
直方图均衡化是指修改图像的像素以增强图像的对比强度的过程。直方图均衡化过程不同于彩色图像的灰度化过程。
...
# 彩色图像处理：不能对3个通道单独地做直方图均衡化，在做其他操作之前，需要将强度信息从颜色信息中分离出来
# 转换到YUV色彩空间
img_yuv = cv.cvtColor(img, cv.COLOR_BGR2YUV)
# 均衡Y通道
img_yuv[:,:,0] = cv.equalizeHist(img_yuv[:,:,0])
# 将其转换回RGB
img_histeq = cv.cvtColor(img_yuv, cv.COLOR_YUV2BGR)
cv.imshow("equalHist_demo", img_histeq)
```
有限制均衡化
