...
```shell
# 步骤
1.找到要合并的分支f1和当前分支dev最近的共同祖先commit点
2.若是这个点不是当前分支dev最新的点，则会把当前分支dev的最新的commit和要合并分支f1的最新的commit合并为一个新的commit，若有冲突，则需要解决冲突，提交解决冲突
3.将以上共同祖先commit点后的所有提交点，按照提交时间（不是push时间）的先后顺序依次放到dev分支上
...
```shell
# 步骤
1.将当前执行rebase分支的所有基于原分支提交点之后的commit取消掉
2.把它们临时保存为补丁(patch)(这些补丁放到".git/rebase"目录中)
3.把当前分支更新为最新的分支，然后把保存的这些path应用到这个分支上
