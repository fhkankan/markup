...
如果你熟悉虚拟 DOM 并且偏爱 JavaScript 的原始力量，你也可以不用模板，[直接写渲染 (render) 函数](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/render-function.html)，使用可选的 JSX 语法。
...
通过使用 [v-once 指令](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/api/#v-once)，你也能执行一次性地插值，当数据改变时，插值处的内容不会更新。但请留心这会影响到该节点上的其它数据绑定：
...
<script> new Vue({ el: '#example1', data: function () { return { rawHtml: 'This should be red.' } } }) </script> {% endraw %}
...
Mustache 语法不能作用在 HTML 特性上，遇到这种情况应该使用 [v-bind 指令](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/api/#v-bind)：
...
在接下来对 [`v-on`](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/events.html#事件修饰符) 和 [`v-for`](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/forms.html#修饰符) 等功能的探索中，你会看到修饰符的其它例子。
