...
很快它就会失控。这也是我们针对需要向任意更深层级的组件提供上下文信息时推荐[依赖注入](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/components-edge-cases.md#依赖注入)的原因。
...
在此之前，在我们描述[访问父级组件实例](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/components-edge-cases.md#访问父级组件实例)的时候，展示过一个类似这样的例子：
...
然而，依赖注入还是有负面影响的。它将你应用程序中的组件与它们当前的组织方式耦合起来，使重构变得更加困难。同时所提供的属性是非响应式的。这是出于设计的考虑，因为使用它们来创建一个中心化规模化的数据跟[使用 `$root`](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/components-edge-cases.md#访问根实例)做这件事都是不够好的。如果你想要共享的这个属性是你的应用特有的，而不是通用化的，或者如果你想在祖先组件中更新所提供的数据，那么这意味着你可能需要换用一个像 [Vuex](https://github.com/vuejs/vuex) 这样真正的状态管理方案了。
...
然而，如果你已经做到了上述的事项仍然发现在极少数的情况下需要手动强制更新，那么你可以通过 [`$forceUpdate`](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/api/#vm-forceUpdate) 来做这件事。
