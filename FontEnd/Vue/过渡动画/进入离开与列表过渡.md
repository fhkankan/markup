...
在这里，我们只会讲到进入、离开和列表的过渡，你也可以看下一节的 [管理过渡状态](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/transitioning-state.html)。
...
2. 如果过渡组件提供了 [JavaScript 钩子函数](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/transitions.md#JavaScript-钩子)，这些钩子函数将在恰当的时机被调用。
...
6. `v-leave-to`: **2.1.8版及以上** 定义离开过渡的结束状态。在离开过渡被触发之后下一帧生效 (与此同时 `v-leave` 被删除)，在过渡/动画完成之后移除。![transition](https://cn.vuejs.org/images/transition.png)
...
我们之后讨论[多个组件的过渡](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/transitions.md#多个组件的过渡)，对于原生标签可以使用 `v-if`/`v-else` 。最常见的多标签过渡是一个列表和描述这个列表为空消息的元素：
...
多个组件的过渡简单很多 - 我们不需要使用 `key` 特性。相反，我们只需要使用[动态组件](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/components.html#动态组件)：
...
- [过渡模式](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/transitions.md#过渡模式)不可用，因为我们不再相互切换特有的元素。
...
[函数式组件](https://github.com/vuejs/cn.vuejs.org/blob/master/src/v2/guide/render-function.html#函数式组件)更适合完成这个任务：
