...
`getattr`函数时python中内置的函数，可以根据对象和方法名动态地获取方法对象。
...
if __name__=="__main__":
	obj = A()
	method_name = "a"
	method = getattr(obj, method_name)  # 获取对象的方法
	res = method()
    print(res)
```
...
`eval`函数时python中内置的函数，可以执行字符串类型的表达式，并返回表达式的值。
...
`exec`函数时python中内置的函数，可以执行字符串类型的表达式，并返回None。
