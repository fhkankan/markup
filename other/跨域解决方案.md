...
如果要发送Cookie，Access-Control-Allow-Origin就不能设为星号，必须指定明确的、与请求网页一致的域名，否则意味着将cookie开发给了所有网站。
...
server {
    ...
    location / {
        ...
        # 当前端只跨域不带cookie时，Access-Control-Allow-Origin可为*，否则意味这将cookie开放给了所有的网站
        add_header Access-Control-Allow-Origin http://www.domain1.com; # 允许跨域请求域名，无cookie时可为*
        add_header Access-Control-Allow-Credentials true;  # 允许传递cookie信息，无cookie时可不写
